...
app.whenReady().then(() => {
  ...
  ipcMain.on('rpa', async (e, json) => {
    ...
    const accountInput = await page.waitForSelector('#loginIdentifier')
    await accountInput.fill('DDKJ205')
    const passwordInput = await page.waitForSelector('#pwd')
    await passwordInput.fill('DD205209')
    const loginBtn = await page.waitForSelector('.next-btn.next-large.next-btn-primary.btn')
    ...
    const close = await mgrPage.waitForSelector('.next-dialog-close')
    await close.click()
    ...
    const title = iframe.locator('div._cardWrapper_1d5bm_1:has-text("内蒙古端点科技有限公司")')
    await title.click()
  })
